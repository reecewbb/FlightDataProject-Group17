class Screen {
  ...
  Screen (int screenType, int widthNo, int heightNo, int ID, String areaName, int zoom)
  {
    ...
    mapImage.resize(SCREENX * zoom, SCREENY * zoom);
    image(mapImage, -widthNo * SCREENX, -heightNo * SCREENY);
    text(areaName, SCREENX/2 - textWidth(areaName)/2, TOP_TEXT_BUFFER);
